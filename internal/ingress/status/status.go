...
type endpointLister interface {
	...
	GetServiceEndpoints(key string) (*apiv1.Endpoints, error)
}
...
func (s statusSync) Run(stopCh chan struct{}) {
	...
	wait.PollUntil(updateInterval, func() (bool, error) {
		s.syncQueue.EnqueueTask(task.GetDummyObject("sync status"))
		endpoints, error := s.getActiveEndpoints()
		if error != nil {
			klog.Errorf("error in updating redis to cleanup old entries - %v", error)
			return false, nil
		}
		s.Config.RedisManager.Clean(endpoints)
		return false, nil
	}, stopCh)
}
...
// getActiveEndpoints gets all the current active ingress endpoints
func (s *statusSync) getActiveEndpoints() ([]string, error) {
	endpoints := []string{}
	...
	for _, ing := range ings {
		for _, rule := range ing.Spec.Rules {
			...
			serviceEndpoints, error := s.EndpointLister.GetServiceEndpoints(svcKey)
			if error != nil {
				klog.Errorf("Error getting services for the ingress %v - %v", ing, error)
